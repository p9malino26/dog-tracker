...
LoRaClass::LoRaClass(long frequency,uint8_t address, int ss, int reset, int dio0, uint8_t* modem_config) :
  _spiSettings(LORA_DEFAULT_SPI_FREQUENCY, MSBFIRST, SPI_MODE0),
  _spi(&LORA_DEFAULT_SPI),
  _ss(ss), _reset(reset), _dio0(dio0),
  _address(address),
  _frequency(frequency),
  _modem_config(modem_config),
  _packetIndex(0),
...
int LoRaClass::init()
{
  ...
  writeRegister(REG_MODEM_CONFIG_1, _modem_config[0]);
  writeRegister(REG_MODEM_CONFIG_2, _modem_config[1]);
  writeRegister(REG_MODEM_CONFIG_3, _modem_config[2]);
...
void LoRaClass::beginPacket() {
  ...
  //write header
  writeRegister(REG_FIFO, 0x0a); //header to
  writeRegister(REG_FIFO, _address); //address
  writeRegister(REG_FIFO, 0x00); //header id
  writeRegister(REG_FIFO, 0x00); //header flags
  // write data
}
void LoRaClass::sendPacket()
{
  modeTx();
}
...
size_t LoRaClass::write(const uint8_t *buffer, size_t size)
{
  ...
  writeRegister(REG_PAYLOAD_LENGTH, _payload_length);
